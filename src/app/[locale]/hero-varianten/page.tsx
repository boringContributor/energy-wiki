  ...
  return (
    <main id="content" className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
      <h1 className="text-3xl font-semibold tracking-tight text-fg">
        {locale === "en" ? "Hero variants" : "Hero-Varianten"}
      </h1>
      <p className="mt-3 max-w-2xl leading-7 text-fg-muted">
        {locale === "en"
          ? "Every scene loops and respects "
        ...
        {locale === "en"
          ? ". The mapping and the home-page scene live in "
        ...
        {HERO_ANIMATIONS.map((variant) => (
          ...
          >
            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 border-b border-border-base bg-surface-2 px-5 py-3.5">
              <h2 className="text-sm font-medium text-fg">{variant.title[locale]}</h2>
            ...
            <div className="grid gap-6 p-5 sm:grid-cols-[1fr_minmax(0,18rem)] sm:items-center">
              <Lottie
                name={variant.name}
                className="w-full"
                ariaLabel={variant.title[locale]}
                fallback={
              ...
              />
              <p className="text-sm leading-6 text-fg-muted">{variant.note[locale]}</p>
            </div>
          </section>
        ))}
      </div>
